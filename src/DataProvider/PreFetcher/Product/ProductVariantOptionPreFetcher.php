...
class ProductVariantOptionPreFetcher implements RestrictedPreFetcherInterface
{
    ...
    public function preFetchData(
        ...
        foreach ($this->repository->findOptionsByProductIds($parentIds, $context) as $result) {
            $this->preFetchedData[$result->getCode()] = $result->getOptionValues()->toArray();
        }
    }
    ...
    public function supports(
        ...
        return is_a($resourceClass, ProductOptionValueInterface::class, true)
            || ($attribute === 'optionValues' && is_a($resourceClass, ProductInterface::class, true));
    }
}
